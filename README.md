...
**Status**: There is currently just one app in this repo, `/apps/doc`, which is the viewer application for symthink documents. There is an `/apps/editor` app for the documents is built on the same stack, but in a private sub-repo currently because it's tied to branding and a backend service.
